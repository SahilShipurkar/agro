...
import { ArrowUpRight, Menu, Search, ShoppingBasket, Trash2, Plus, Minus, MessageSquare, CheckCircle, Receipt, ShieldCheck } from "lucide-react"; 
import { 
  Sheet, 
  SheetContent, 
  SheetHeader, 
...
export function CommerceHero({
  ...
  const [cartItems, setCartItems] = useState<CartItem[]>([
    {
      id: "DUGDHSAMRUDHI SARKI PEND-40 kg Bag",
      name: "DUGDHSAMRUDHI SARKI PEND",
      variant: "40 kg Bag",
      quantity: 1,
      unitPrice: 1800,
  ...
  const handleAddToCart = (newItem: { name: string; variant: string; quantity: number; unitPrice: number; image: string; category: string }) => {
  ...
  const generateWhatsAppMessage = () => {
    if (cartItems.length === 0) return "https://wa.me/918237795424";
    let text = "Hello Navbharat Agro Services! I would like to place an order for the following products:\n\n";
    cartItems.forEach((item, index) => {
      const lineTotal = item.unitPrice * item.quantity;
      text += `${index + 1}. ${item.name} (${item.variant}) - Qty: ${item.quantity} x ₹${item.unitPrice} = ₹${lineTotal.toLocaleString('en-IN')}\n`;
    });
    text += `\n---------------------------------\n`;
    text += `TOTAL FINAL BILL: ₹${totalBillAmount.toLocaleString('en-IN')}\n`;
    text += `Dispatch: Direct Farm Delivery\n\nPlease confirm availability and payment options.`;
    return `https://wa.me/918237795424?text=${encodeURIComponent(text)}`;
  };
  ...
  return ( 
    <div className="w-full relative container px-2 sm:px-4 mx-auto max-w-7xl min-h-screen py-6 font-['Inter',sans-serif]"> 
      <div className="mt-4 bg-[#EFFDF0] border-2 border-[#123814]/15 rounded-3xl relative shadow-xl overflow-hidden p-4 sm:p-8"> 
        <header className="flex items-center justify-between gap-4 border-b border-[#123814]/10 pb-4"> 
          <div className="w-full lg:w-2/3 bg-white/90 backdrop-blur-sm p-3 sm:p-4 rounded-2xl flex items-center justify-between border border-[#123814]/10 shadow-sm"> 
            <div 
            ...
            <nav className="hidden lg:flex items-center justify-end gap-6 w-full ml-6"> 
              ...
              {/* Navbar Cart Button with Badge */}
              <Button 
                variant="ghost" 
                size="icon" 
                onClick={() => setIsCartOpen(true)} 
                className="cursor-pointer relative hover:text-[#E86A10]"
              > 
                <ShoppingBasket className="w-5 h-5 text-[#123814]" /> 
                {totalCartCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-4.5 h-4.5 bg-[#E86A10] text-white text-[10px] font-black rounded-full flex items-center justify-center shadow-xs">
                    {totalCartCount}
                  </span>
                )}
              </Button> 
              ...
              <SheetContent side="left" className="w-[300px] sm:w-[400px] p-0 bg-white/95 backdrop-blur-md"> 
                <SheetHeader className="p-6 text-left border-b"> 
                  <SheetTitle className="flex items-center gap-3"> 
                    <img src="/Whitte Circle logo(3).png" alt="Logo" className="w-10 h-10 object-contain rounded-full" />
      ...
      {/* Shopping Cart Drawer Sheet & Final Bill Calculation */}
      <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
        <SheetContent side="right" className="w-[340px] sm:w-[460px] p-0 bg-white/98 backdrop-blur-xl flex flex-col justify-between border-l-2 border-[#123814]/20">
          <div>
            <SheetHeader className="p-6 border-b border-[#123814]/10 bg-[#EFFDF0]">
              <SheetTitle className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-[#123814] font-extrabold text-lg">
              ...
              {cartItems.length === 0 ? (
                <div className="py-12 text-center space-y-3">
                  ...
                  <Button 
                    ...
                    className="bg-[#123814] text-white rounded-full px-5 py-2 text-xs font-bold"
                  >
                ...
                cartItems.map((item) => {
                  const lineTotal = item.unitPrice * item.quantity;
                  return (
                    <div key={item.id} className="flex items-center justify-between bg-[#EFFDF0]/80 p-3.5 rounded-2xl border border-[#123814]/15 shadow-xs">
                      <div className="flex items-center gap-3">
                        <img src={item.image} alt={item.name} className="w-14 h-14 object-contain rounded-xl bg-white p-1 border border-[#123814]/10 shadow-xs" />
                        <div>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex items-center bg-white border border-[#123814]/20 rounded-full px-1 py-0.5 shadow-xs">
                          <button
                            ...
                            className="w-5 h-5 rounded-full bg-gray-100 hover:bg-gray-200 text-[#123814] flex items-center justify-center font-bold text-xs"
                          >
          ...
          {/* Cart Final Bill Calculation Summary Footer */}
          {cartItems.length > 0 && (
            <div className="p-6 border-t-2 border-[#123814]/15 bg-[#EFFDF0] space-y-4">
              <div className="bg-white p-4 rounded-2xl border border-[#123814]/15 shadow-xs space-y-2">
              ...
              <a
                ...
                className="w-full bg-[#25D366] hover:bg-[#1eb956] text-white py-3.5 rounded-full font-bold text-sm shadow-lg flex items-center justify-center gap-2 transition-all cursor-pointer"
              >
                <MessageSquare className="w-4.5 h-4.5 fill-white" />
                Confirm Order & Pay via WhatsApp (₹{totalBillAmount.toLocaleString('en-IN')})
              </a>
            </div>
          )}
        </SheetContent>
